...
class _AddServerModalState extends State<AddServerModal> {
  ...
  void initState() {
    if (widget.server != null) {
      ...
      connectionType = widget.server!.connectionMethod == 'https'
  ...
  Widget build(BuildContext context) {
    ...
    String getErrorMessage(AuthStatus status) {
      if (status == AuthStatus.invalidCredentials)
        return AppLocalizations.of(context)!.invalidUsernamePassword;
      if (status == AuthStatus.manyAttepts)
        return AppLocalizations.of(context)!.tooManyAttempts;
      if (status == AuthStatus.socketException ||
          status == AuthStatus.timeoutException)
        return AppLocalizations.of(context)!.cantReachServer;
      if (status == AuthStatus.serverError)
        return AppLocalizations.of(context)!.serverError;
      if (status == AuthStatus.handshakeException)
    ...
    void connect() async {
      ...
      Server serverObj = Server(
          ...
          port:
              portController.text != '' ? int.parse(portController.text) : null,
      ...
      if (serverObj.user != null && serverObj.password != null) {
        serverObj.authToken =
            encodeBase64UserPass(serverObj.user!, serverObj.password!);
      }
      ...
      statusProvider.setServerStatusData(data: status);
    ...
    void edit() async {
      ...
      if (serverSaved != null) {
        setState(() => isConnecting = false);
        appConfigProvider.addLog(AppLog(
    ...
    List<Widget> form() {
      return [
        ...
        FormTextField(
          ...
          onChanged: (value) {
            if (value != '') {
              ...
              setState(
                  () => nameError = AppLocalizations.of(context)!.nameNotEmpty);
            }
            validateData();
          },
        ...
        SegmentedButtonSlide(
          ...
          onChange: (v) =>
              setState(() => connectionType = ConnectionType.values[v]),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Flexible(
                    child: Text(
                        AppLocalizations.of(context)!.redirectHttpsWarning))
              ],
            ),
          ),
        ),
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 16),
                  Flexible(
                      child: Text(AppLocalizations.of(context)!.sslWarning))
                ],
              ),
            ),
          ),
        ...
        FormTextField(
          ...
          onChanged: (v) {
            setState(() =>
                ipDomainError = validateAddress(context: context, value: v));
            validateData();
          },
          isConnecting: isConnecting,
        ),
        ...
        FormTextField(
          ...
          onChanged: (v) {
            setState(
                () => pathError = validateSubroute(context: context, value: v));
            validateData();
          },
        ...
        FormTextField(
          ...
          onChanged: (v) {
            setState(
                () => portError = validatePort(context: context, value: v));
            validateData();
          },
          isConnecting: isConnecting,
        ),
