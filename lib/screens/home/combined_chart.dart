    ...
    if (statusProvider.serverStatus != null) {
      final data = CombinedChartData(
        ...
        blockedFilters: appConfigProvider.hideZeroValues == true
            ? removeZero(statusProvider.serverStatus!.stats.blockedFiltering) !=
                    null
                ? CombinedChartItem(
      ...
      final hoursInterval =
          statusProvider.serverStatus!.stats.timeUnits == "days" ? 24 : 1;

      List<DateTime> dateTimes = [];
      DateTime currentDate = DateTime.now().subtract(Duration(
          hours: hoursInterval *
                  statusProvider.serverStatus!.stats.dnsQueries.length +
              1));
      for (var i = 0;
          i < statusProvider.serverStatus!.stats.dnsQueries.length;
          i++) {
                        ...
                        children: [
                          _Legend(
                            ...
                            primaryValue: intFormat(
                                ...
                                Platform.localeName),
                            secondaryValue:
                                "${doubleFormat(statusProvider.serverStatus!.stats.avgProcessingTime * 1000, Platform.localeName)} ms",
                          ),
                          const SizedBox(height: 16),
                          if (data.blockedFilters != null)
                            _Legend(
                              label: data.blockedFilters!.label,
                              color: data.blockedFilters!.color,
                              primaryValue: intFormat(
                                  statusProvider
                                      .serverStatus!.stats.numBlockedFiltering,
                                  Platform.localeName),
                              secondaryValue:
                                  "${statusProvider.serverStatus!.stats.numDnsQueries > 0 ? doubleFormat((statusProvider.serverStatus!.stats.numBlockedFiltering / statusProvider.serverStatus!.stats.numDnsQueries) * 100, Platform.localeName) : 0}%",
                            ),
                        ],
                      ),
                    )),
